  ...
  * Intent detection from natural language messages
    * Multi-turn conversation state (per user phone number)
      * Response formatting
      ...
      Fixes vs original:
        1. Use dateparser.search.search_dates() to extract dates embedded in full
             sentences like "what tee times are available this Friday".
               2. Save AWAITING_DATE session state when asking for a date, so the
                    follow-up reply (e.g. "April 3") is correctly handled.
                    """
...
IDLE          = "idle"
AWAITING_DATE = "awaiting_date"   # bot asked for a date; waiting for reply
SHOW_GOLF     = "show_golf"
...
def _detect_intent(msg: str) -> Dict[str, Any]:
      """
          Parse a free-text message and return an intent dict.
              Returns: {type, date_str, time_pref, selection, duration_min, party_size}
                  """
      ...
      result: Dict[str, Any] = {
          ...
          "selection":    None,   # int index or time string
          "duration_min": 60,
          "party_size":   2,
      }
    ...
    # -- Date parsing ----------------------------------------------------------
    # Use search_dates to find dates embedded in full sentences like
    # "what tee times are available this Friday" or "book me April 5".
    ...
    # Fall back to dateparser.parse for standalone strings like "April 3"
    if not date_found:
...
# -- Shared slot-lookup helper -------------------------------------------------
async def _lookup_slots(booking_type, date_str, intent, foretees):
      """Fetch slots and return (slots, reply_text, next_step)."""
...
async def handle_message(
    ...
    # -- We asked for a date last turn; treat this reply as the date -----------
    if session.get("step") == AWAITING_DATE:
              date_str = intent.get("date_str")
              if not date_str:
                            return (
                                              "I didn't catch that date. "
                                              "Please try again (e.g. _April 3_ or _this Saturday_)."
                            )
    ...
    # -- In a booking flow: user is picking a slot -----------------------------
    ...
    if not date_str:
              ...
              # KEY FIX: save context so the follow-up date reply is handled correctly
              _sessions[phone] = {
                            "step":         AWAITING_DATE,
                            "type":         booking_type,
        ...
        return (
                      f"What date would you like to check {type_word} for? "
                      f"(e.g. _this Saturday_ or _April 12_)"
        )
