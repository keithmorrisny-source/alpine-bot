...
# ForeTees login page (shared across all modules)
...
class ForeTees:
    ...
    async def _is_error_page(self, page: Page) -> bool:
        """Return True if the page is showing the ForeTees Access Error."""
        try:
            body = await page.evaluate("document.body.innerText")
            return "Access Error" in body or "login" in page.url.lower()
        except Exception:
            return True

    async def _ensure_logged_in(self, page: Page, base_url: str) -> bool:
        """
        Navigate to the module's main page and log in if needed.
        Returns True on success.
        
        Key facts about the ForeTees login form:
          - URL:      https://ccapp.foretees.com/v5/servlet/LoginPrompt?cn=alpinecc
          - Username: input[name='user_name']
          - Password: input[name='password']
          - Submit:   input[value='Sign In']
        """
        # First try navigating directly to the base section
        try:
            await page.goto(
                f"{base_url}/Member_select",
                wait_until="networkidle",
                timeout=30000,
            )
        except Exception as exc:
            print(f"[ForeTees] Initial nav error: {exc}")

        # If already on a valid member page, we're done
        if not await self._is_error_page(page):
            print("[ForeTees] Already logged in")
            return True

        # Need to log in via the real login page
        print("[ForeTees] Not logged in, attempting login...")
        try:
            await page.goto(LOGIN_URL, wait_until="networkidle", timeout=15000)
            await page.wait_for_selector('input[name="user_name"]', timeout=8000)
            ...
            # Navigate to the target module after login
            await page.goto(
                f"{base_url}/Member_select",
                wait_until="networkidle",
                timeout=30000,
            ...
            success = not await self._is_error_page(page)
            print(f"[ForeTees] Login {'succeeded' if success else 'failed'} - URL: {page.url}")
            return success
        except Exception as exc:
    ...
    async def get_tee_times(self, date_str: str) -> List[Dict[str, Any]]:
        ...
        try:
            if not await self._ensure_logged_in(page, GOLF_BASE):
                print("[ForeTees] get_tee_times: login failed, returning empty")
                return []

            sheet_url = f"{GOLF_BASE}/Member_sheet?calDate={date_str}&course=&displayOpt=0"
            print(f"[ForeTees] Fetching tee sheet: {sheet_url}")
            await page.goto(sheet_url, wait_until="networkidle", timeout=30000)

            print(f"[ForeTees] Tee sheet URL after nav: {page.url}")
            page_title = await page.title()
            print(f"[ForeTees] Tee sheet title: {page_title}")

            # Collect all anchor elements that look like tee-time links
            time_links = await page.locator('a[href="#"]').all()
            print(f"[ForeTees] Found {len(time_links)} a[href='#'] elements")
            ...
            print(f"[ForeTees] Returning {len(available)} tee times for {date_str}")
            return available
        finally:
            await page.close()

    async def book_tee_time(self, date_str: str, time_str: str) -> Dict[str, Any]:
        """Book a tee time. Player 1 is pre-filled as the logged-in member."""
        page = await self._new_page()
        try:
            if not await self._ensure_logged_in(page, GOLF_BASE):
                return {"success": False, "message": "Login failed — cannot book tee time."}
    ...
    async def get_tennis_courts(self, date_str: str) -> List[Dict[str, Any]]:
        ...
        page = await self._new_page()
        try:
            if not await self._ensure_logged_in(page, TENNIS_BASE):
                return []
    ...
    async def book_tennis_court(
        ...
        try:
            if not await self._ensure_logged_in(page, TENNIS_BASE):
                return {"success": False, "message": "Login failed — cannot book tennis court."}
            ...
            for row in rows:
                ...
                if time_text.upper() != time_str.upper():
                    continue
                target_cell = cells[court_num] if len(cells) > court_num else None
                if target_cell:
    ...
    async def get_dining_slots(self, date_str: str) -> List[Dict[str, Any]]:
        ...
        page = await self._new_page()
        try:
            if not await self._ensure_logged_in(page, DINING_BASE):
                return []
    ...
    async def book_dining(
        ...
        try:
            if not await self._ensure_logged_in(page, DINING_BASE):
                return {"success": False, "message": "Login failed — cannot book dining."}
